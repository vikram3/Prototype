...
func _check_coins(context: Node) -> bool:
	...
	return int(objective.coins_collected) >= required_coins
...
func _check_objective_complete(context: Node) -> bool:
	...
	if not objective.has_method("is_complete"):
		return false
...
func _check_player_hidden(context: Node) -> bool:
	...
	if not player.has_method("is_hidden"):
		return false
...
func _check_event(context: Node) -> bool:
	...
	if not context.has_method("has_story_event"):
		return false

	return context.has_story_event(event_name)


func _find_objective(context: Node) -> Node:
	if context != null and context.has_node("Objective"):
		return context.get_node("Objective")

	var scene := get_tree().current_scene

	if scene != null and scene.has_node("Objective"):
		return scene.get_node("Objective")

	return null


func _find_player(context: Node) -> Node:
	if context != null and context.has_method("_find_player"):
